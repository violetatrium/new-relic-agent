    ...
    def update_extended_stats(self,):
        ...
        if caches:
            ...
            if 'cache/resp/hit' in self.deltas.keys():
                cache_resp_cached = (self.deltas['cache/resp/hit'] + self.deltas['cache/resp/stale'] + self.deltas['cache/resp/updating'] + self.deltas['cache/resp/revalidated'])
                cache_resp_uncached = (self.deltas['cache/resp/miss'] + self.deltas['cache/resp/expired'] + self.deltas['cache/resp/bypass'])
            if (cache_resp_cached + cache_resp_uncached) > 0:
...
class NginxNewRelicAgent():
    ...
    def run(self):
        ...
        LOG.info('starting with %d configured data sources, poll_interval=%d', len(self.sources), self.poll_interval)
